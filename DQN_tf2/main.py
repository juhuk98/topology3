...
def train(max_episode_num, n_agents, topology, node_type, random, batch_size, path):
    ...
    for ep in range(int(max_episode_num)):
        ...
        while not epi_done:
            ...
            if step==100: done = np.array([1 for i in done])
            ...
            if not node_type:
                multi.store_memory(state, action, train_reward, next_state, done)
        ...
        if ep >= store_period:
            if ep % target_update_cycle == 0 or ep==int(max_episode_num)-1:
                multi.save_checkpoint('./result/'+path+'/model/')
                multi.update_target_model()
            multi.epsilon_decay()
